...
#         self.config = config
#         self.scam_type = 'ecommerce'  # Default scam type

#     def set_scam_type(self, scam_type):
#         """Set the scam type ('ecommerce', 'phishing', 'government officials impersonation')."""
#         if scam_type in self.config['demographic_configs']:
#             self.scam_type = scam_type
#         else:
#             raise ValueError(f"Invalid scam type: {scam_type}. Choose from {list(self.config['demographic_configs'].keys())}")
...
#         """Generate nationality based on weights for the current scam type."""
#         weights = self.config['demographic_configs'][self.scam_type]['nationality_weights']
...
#         """Generate race based on nationality and weights for the current scam type."""
#         race_weights = self.config['demographic_configs'][self.scam_type]['race_weights'][nationality]
#         return self._weighted_choice(race_weights)['race']

#     def sex(self):
#         """Generate sex based on weights for the current scam type."""
#         weights = self.config['demographic_configs'][self.scam_type]['sex_weights']
#         return random.choices(
#             list(weights.keys()),
#             weights=list(weights.values()),
#             k=1
...
#     def vic_first_name(self, race, sex):
...
#     def vic_last_name(self, race):
#         """Generate last name based on race."""
#         names = self.config['race_name_lists'].get(race, self.config['race_name_lists']['Other'])
#         return random.choice(names['last_names'])

#     def dob(self):
#         """Generate date of birth based on age ranges for the current scam type."""
#         age_range = self._weighted_choice(self.config['demographic_configs'][self.scam_type]['age_ranges'])
...
#     def occupation(self):
#         """Generate occupation based on weights for the current scam type."""
#         return self._weighted_choice(self.config['demographic_configs'][self.scam_type]['occupations'])['occupation']
...
#         """Generate a single victim profile."""
#         nat = self.nationality()
#         race = self.race(nat)
#         sex = self.sex()
#         first_name = self.vic_first_name(race, sex)
#         last_name = self.vic_last_name(race)
#         dob = self.dob()
#         return {
#             'scam_type': self.scam_type,
#             'vic_first_name': first_name,
#             'vic_last_name': last_name,
#             'vic_sex': sex,
#             'vic_dob': dob,
#             'vic_contact_no': self.victim_contact_no(),
#             'vic_blk': self.blk(),
#             'vic_street': self.street(),
#             'vic_unit_no': self.unit_no(),
#             'vic_postal_code': self.postal_code(),
#             'vic_occupation': self.occupation(),
#             'vic_nationality': nat,
#             'vic_race': race,
#             'vic_email': self.email(first_name, last_name),
#             'vic_nric': self.nric(nat, dob)
#         }

# class VictimProfileGenerator:
#     """A class to generate victim profiles for different scam types using Faker."""
    
#     def __init__(self, config_path='config/victim_config.yaml'):
#         with open(config_path, 'r') as f:
#             self.config = yaml.safe_load(f)
#         self.fake = Faker()
#         self.fake.add_provider(CustomSingaporeProvider(self.fake, config=self.config))
#         self.settings = get_settings()
#         self.default_output_file = self.settings.data.victim_details_csv

#     def transform_victim_string(self, value) -> str:
#         """Capitalize string values or return NA for empty fields."""
#         if isinstance(value, str) and value != "":
#             return value.upper()
#         return "NA" if pd.isna(value) else value
    
#     def generate_profiles_dataframe(self, total_records: int, proportions: dict, output_file: str = None) -> pd.DataFrame:
#         """Generate victim profiles based on proportions and return as a Pandas DataFrame with transformed string values."""
#         if not abs(sum(proportions.values()) - 1.0) < 1e-6:
#             raise ValueError("Proportions must sum to 1.0")
#         for scam_type in proportions:
#             if scam_type not in self.config['demographic_configs']:
#                 raise ValueError(f"Invalid scam type: {scam_type}. Choose from {list(self.config['demographic_configs'].keys())}")
...
#             profile_counts[max_proportion_scam] += total_records - total_assigned

#         profiles = []
#         for scam_type, count in profile_counts.items():
#             self.fake.set_scam_type(scam_type)
#             for _ in range(count):
#                 profile = self.fake.generate_victim()
#                 profiles.append(profile)

#         df = pd.DataFrame(profiles)
#         columns = [
#             'scam_type', 'vic_first_name', 'vic_last_name', 'vic_nric', 'vic_sex',
#             'vic_dob', 'vic_nationality', 'vic_race', 'vic_occupation', 'vic_contact_no',
#             'vic_email', 'vic_blk', 'vic_street', 'vic_unit_no', 'vic_postal_code'
#         ]
#         df = df[columns]

#         case_sensitive_column = ['vic_dob','vic_contact_no','vic_email']
    
#         for col in columns:
#             if col not in case_sensitive_column:
#                 df[col] = df[col].apply(self.transform_victim_string)

#         return df

#     def save_to_csv(self, df: pd.DataFrame, output_file: str = None) -> None:
#         """Save the victim DataFrame to a CSV file."""
#         if output_file is None:
#             output_file = self.default_output_file
#         if not df.empty and output_file:
#             os.makedirs(os.path.dirname(output_file), exist_ok=True)
#             df.to_csv(output_file, index=False)
#             print(f"Saved {len(df)} victim profiles to {output_file}")
...
#     victim_generator = VictimProfileGenerator()
#     victim_df = victim_generator.generate_profiles_dataframe(
#         total_records,
#         proportions,
#         output_file=None
#     )
#     victim_generator.save_to_csv(victim_df)
#     print(f"Total records generated: {len(victim_df)}")
#     print("Records per scam type:")
#     print(victim_df['scam_type'].value_counts())
#     print("\nSample records:")
#     print(victim_df.head())
