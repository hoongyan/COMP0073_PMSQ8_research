...
class PersonalisationScoreCalculator:
    ...
    def preprocess(self):
        ...
        for convo_id, group in grouped:
            ...
            # Determine segment (underscore-separated)
            ...
            overall_norms = group['overall_norm'].dropna().tolist()  # List of per-turn norms
    ...
    #         # Return raw scores for advanced analysis/charts
    #         'raw_scores': {
    ...
    def compute_metrics(self, preprocessed):
        ...
        # New: Collect lengths by LLM model
        by_llm_lengths = defaultdict(list)
        
        for data in preprocessed.values():
            # New: Append length for this conversation's model
        ...
        # New: Compute stats for turns per model
    ...
    def generate_appropriateness_charts(self, preprocessed):
        ...
        models = sorted(plot_data['llm_model'].unique())  # Sort for consistent order
        self.logger.info(f"Unique models: {models}")  # Debug: log model names
        import matplotlib.colors as mcolors
        color_list = list(mcolors.TABLEAU_COLORS.values())
       # Initialize colors with Tableau palette for other models
        ...
        for ind in indicators:
            ...
            for model in models:
                ...
                # Plot score line (muted with alpha)
                ax.plot(x, y, label=model, color=color, alpha=0.3, linewidth=1)
                
                # Trend line (vivid, bold)
                X = np.array(x).reshape(-1, 1)
                reg = LinearRegression().fit(X, y)
                trend = reg.predict(X)
                ax.plot(x, trend, linestyle='--', color=color, label=f'{model} Trend', linewidth=2)
            
            ax.set_title(f'Appropriateness Score Over Sorted Conversations ({ind.capitalize()})', fontsize=20)  # Larger title font
            ax.set_xlabel('Conversation Index (Sorted by ID)', fontsize=20)  # Larger x-axis label font
            ax.set_ylabel('Normalized Score', fontsize=20)  # Larger y-axis label font
            ax.set_ylim(0.2, 1)  # Set y-axis to start at 0.2
            ax.tick_params(axis='both', labelsize=18)  # Larger tick label (scale) font
    ...
    def generate_charts(self, metrics, preprocessed):
        ...
        # Recollect overall_scores for boxplots
        overall_scores = defaultdict(list)
        for data in preprocessed.values():
            for ind in all_indicators:
                score = data.get(ind_to_key[ind])
                if score is not None and not np.isnan(score):  # Safe check for None/nan
                    overall_scores[ind].append(score)
        
        # Define segment order and maps (from user_profile_metrics.py)
        desired_order = [
    ...
    def generate_length_impact_charts(self, preprocessed):
        ...
        # Color assignment (consistent with other charts)
        import matplotlib.colors as mcolors
        color_list = list(mcolors.TABLEAU_COLORS.values())
        # Initialize colors with Tableau palette for other models
        colors = {model: color_list[i % len(color_list)] for i, model in enumerate(models)}
        ...
        # Find global max_length across all models
        ...
        ax.legend(fontsize=16, ncol=2)  # Adjust ncol if many models
        ax.grid(True)

        # Twin axis for overall convo count (sum across models)
...
if __name__ == "__main__":
    ...
    # ie_csv = "evaluation/results/communication_appropriateness/phase_2/evaluated_autonomous_conversations_profile_rag_ie.csv"
    ...
    # Quick check print for average turns per model
